...
void TextureManager::RenderFrame(int x, int y, int width, int height, int row, int end, int currentframe, int framecount, int numAction, double angle, SDL_RendererFlip flip)
{
...
void TextureManager::RenderFrame(int x, int y, int width, int height, int row, int start, int end, int currentframe, int framecount, int numAction, double angle, SDL_RendererFlip flip)
{
	float srcWidth = originWidth / framecount;
	float srcHeight = originHeight / numAction;
	SDL_Rect srcRect = { srcWidth * currentframe, srcHeight * (row - 1), srcWidth, srcHeight }; //pointer to source rect(the area and position where you get the sprite on the texture).
